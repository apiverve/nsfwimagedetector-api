...
class NsfwimagedetectorAPIClientError(Exception):
...
class ValidationError(NsfwimagedetectorAPIClientError):
...
class NsfwimagedetectorAPIClient:
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the NsfwimagedetectorAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises NsfwimagedetectorAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/nsfwimagedetector'
        self.headers = {
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise NsfwimagedetectorAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[NsfwimagedetectorAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns NSFWImageDetectorData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises NsfwimagedetectorAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise NsfwimagedetectorAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise NsfwimagedetectorAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise NsfwimagedetectorAPIClientError(f"Request failed: {str(e)}")
    ...
    def execute_with_file(self, files: Dict[str, Any], params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self._log(f"Request exception: {str(e)}")
            raise NsfwimagedetectorAPIClientError(f"File upload failed: {str(e)}")
    ...
    def execute_with_url(self, url: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if not url or not isinstance(url, str):
            raise NsfwimagedetectorAPIClientError("URL must be provided as a string.")

        if not re.match(r'^https?://.+', url, re.IGNORECASE):
            raise NsfwimagedetectorAPIClientError("Invalid URL format. URL must start with http:// or https://")
